...
import { orderTableByClinicId } from "@features/clinic/api/clinic.api";
...
registerTable("clinic-orders", () =>
  createTableSchema<OrderModel>({
    columns,
    fetch: async (opts: FetchTableOpts & Record<string, any>) => await orderTableByClinicId(opts.clinicId, opts),
    initialPageSize: 10,
