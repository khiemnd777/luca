...
import { orderTableByPatientId } from "@features/patient/api/patient.api";
...
const columns: ColumnDef<OrderModel>[] = [
  ...
  { key: "processNameLatest", header: "Công đoạn" },
  { key: "totalPrice", type: "currency", header: "Thành tiền", sortable: true },
...
registerTable("patient-orders", () =>
  createTableSchema<OrderModel>({
    columns,
    fetch: async (opts: FetchTableOpts & Record<string, any>) => await orderTableByPatientId(Number(opts.patientId), opts),
    initialPageSize: 10,
    initialSort: { by: "updated_at", dir: "desc" },
    onView: (row) => { navigate(`/order/${row.id}`); },
    async onDelete(row) {
      const resolvedOrderItemId = await getLatestOrderItemIdByOrderId(row.id);
      await unlink(row.id, resolvedOrderItemId);
      reloadTable("patient-orders");
    },
  })
);
