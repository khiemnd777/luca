...
SELECT
  ...
  NULLIF(concat_ws('|', b.code, b.name, b.category_name), ''),
  NULL,
  jsonb_build_object('code', COALESCE(b.code, '')),
...
SELECT
  ...
  NULLIF(concat_ws('|', rm.code, rm.name, rm.category_name), ''),
  NULL,
  jsonb_build_object('code', COALESCE(rm.code, '')),
...
SELECT
  ...
  NULLIF(concat_ws('|', t.code, t.name, t.category_name), ''),
  NULL,
  jsonb_build_object('code', COALESCE(t.code, '')),
...
SELECT
  ...
  NULLIF(concat_ws('|', rt.code, rt.name, rt.category_name), ''),
  NULL,
  jsonb_build_object('code', COALESCE(rt.code, '')),
