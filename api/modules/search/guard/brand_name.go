...
func init() {
	logger.Debug("[GuardSearch] Register BrandName")
	search.RegisterGuard("brand_name", func(ctx search.GuardCtx, rows []model.Row) []model.Row {
	...
	search.RegisterGuard("brand", func(ctx search.GuardCtx, rows []model.Row) []model.Row {
		if !rbac.HasAnyPerm(ctx.Perms, "product.search") {
			return []model.Row{}
		}
		out := make([]model.Row, 0, len(rows))
