...
import (
	...
	"github.com/khiemnd777/noah_api/modules/main/features/clinic/service"
...
func (h *ClinicHandler) RegisterRoutes(router fiber.Router) {
	...
	app.RouterGet(router, "/:dept_id<int>/clinic/:clinic_id<int>/orders", h.ListOrdersByClinicID)
...
func (h *ClinicHandler) ListOrdersByClinicID(c *fiber.Ctx) error {
	...
	deptID, _ := utils.GetDeptIDInt(c)
	res, err := h.svc.ListOrdersByClinicID(c.UserContext(), deptID, clinicID, q)
	if err != nil {
		return client_error.ResponseError(c, fiber.StatusInternalServerError, err, err.Error())
	}
	return c.Status(fiber.StatusOK).JSON(res)
