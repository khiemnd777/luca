...
type RawMaterialHandler struct {
	svc  service.RawMaterialService
	deps *module.ModuleDeps[config.ModuleConfig]
}

func NewRawMaterialHandler(svc service.RawMaterialService, deps *module.ModuleDeps[config.ModuleConfig]) *RawMaterialHandler {
	return &RawMaterialHandler{svc: svc, deps: deps}
}

func (h *RawMaterialHandler) RegisterRoutes(router fiber.Router) {
	app.RouterGet(router, "/:dept_id<int>/raw_material/list", h.List)
	app.RouterGet(router, "/:dept_id<int>/raw_material/search", h.Search)
	app.RouterGet(router, "/:dept_id<int>/raw_material/:id<int>", h.GetByID)
	app.RouterPost(router, "/:dept_id<int>/raw_material", h.Create)
	app.RouterPut(router, "/:dept_id<int>/raw_material/:id<int>", h.Update)
	app.RouterDelete(router, "/:dept_id<int>/raw_material/:id<int>", h.Delete)
}

func (h *RawMaterialHandler) List(c *fiber.Ctx) error {
...
func (h *RawMaterialHandler) Search(c *fiber.Ctx) error {
	if err := rbac.GuardAnyPermission(c, h.deps.Ent.(*generated.Client), "product.view"); err != nil {
		return client_error.ResponseError(c, fiber.StatusForbidden, err, err.Error())
	}
...
func (h *RawMaterialHandler) GetByID(c *fiber.Ctx) error {
	if err := rbac.GuardAnyPermission(c, h.deps.Ent.(*generated.Client), "product.view"); err != nil {
		return client_error.ResponseError(c, fiber.StatusForbidden, err, err.Error())
	}
...
func (h *RawMaterialHandler) Create(c *fiber.Ctx) error {
	...
	var payload model.RawMaterialDTO
...
func (h *RawMaterialHandler) Update(c *fiber.Ctx) error {
...
func (h *RawMaterialHandler) Delete(c *fiber.Ctx) error {
